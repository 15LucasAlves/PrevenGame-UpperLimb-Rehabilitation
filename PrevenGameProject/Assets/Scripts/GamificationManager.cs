...
public class GamificationManager : MonoBehaviour
{
    ...
    private int   _pontuacaoTotal = 0;
    private int   _dardosLancados = 0;
    ...
    void LancarDardo(float pct)
    {
        ...
        _dardosLancados++;
        _pontuacaoTotal += aro * 20; // aro 1→20 … aro 5→100
    ...
    void AtualizarHUD()
    {
        if (TextoPontuacao) TextoPontuacao.text = $"Pontuação: {_pontuacaoTotal}";
        if (TextoDardos)    TextoDardos.text    = $"Dardos: {NumRepeticoes - _dardosLancados}/{NumRepeticoes}";
    }
