...
public class PrevenGameManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI   TextoPercentagem; // canto superior esquerdo — % média viva
    public TextMeshProUGUI   TextoRepeticao;   // canto inferior direito — reps x/X
    ...
    public Button            BotaoVoltarMenu;        // volta ao MainMenu
    public Image[]           ImagensExercicio;       // [4] imagem dentro de cada card
    public ExercicioDemoLoop DemoExercicio;
    public Sprite[][]        SpritesExercicios;      // [4] — índice == (int)TipoExercicio
    ...
    void IniciarExercicio(ExercicioConfig config)
    {
        ...
        if (DemoExercicio != null && SpritesExercicios != null)
        {
            int i = (int)config.Tipo;
            if (i < SpritesExercicios.Length && SpritesExercicios[i] != null)
                DemoExercicio.DefinirSprites(SpritesExercicios[i]);
        }
    ...
    void AtualizarHUD()
    {
        if (TextoPercentagem)
        {
            float pct = _waypointsAtingidos > 0
                ? (_pontuacaoAcumulada / _waypointsAtingidos) * 100f
                : 0f;
            TextoPercentagem.text = $"{pct:F0} %";
        }

        if (TextoRepeticao)
            TextoRepeticao.text = $"{_repAtual + 1}/{NumRepeticoes}";
